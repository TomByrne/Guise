...
class DropDownTrait extends DisplayTrait
{
	...
	private function createSelectElement():Element {
		_select = cast Browser.document.createElement("select");
		return _select;
	}
	...
	private function compileItems():Void {
		...
		for (textLabel in _textLabels) {
			var option:OptionElement = cast Browser.document.createElement("option");
	...
	private function onTextChanged(from:ITextLabel):Void {
		...
		var option:OptionElement = cast _select.options[index];
