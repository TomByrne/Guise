...
{
	var defToTexture:Hash<BitmapData>;
	var defToTextures:Hash<Array<BitmapData>>;
	...
	public function new(img:BitmapData)
	{
		...
		defToTexture = new Hash<BitmapData>();
		defToTextures = new Hash<Array<BitmapData>>();
	...
	{
		return defToTexture.get(name);
	}
	public function getTextures(name:String):Array<BitmapData>
	{
		var ret = defToTextures.get(name);
		if (ret != null) {
			return ret;
		}else {
			var indices:Array<Int> = getAnim(name);
			ret = new Array<BitmapData>();
			for (i in indices) {
				ret.push(bmps[i]);
			}
			defToTextures.set(name, ret);
			return ret;
		}
